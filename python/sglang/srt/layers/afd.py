...
def model_forward_afd(
    ...
    def forward_F(layer_id: int, mirco_batch_idx: int):
        # logger.warning(f"cll ---------- forward forward_F layer:{layer_id}")
...
class AFDCommunicator(LayerCommunicator):
    ...
    def prepare_mlp(
        ...
        logger.warning(f"=============0 AFDCommunicator prepare_mlp : {get_attention_tp_rank()}, {hidden_states}")
        if self.perspective == AFDPerspective.AFD_PERSPECTIVE_FFN:
            logger.warning(f"=============0.1 AFDCommunicator prepare_mlp : {get_attention_tp_rank()}, {get_tensor_communicator()}")
            hidden_states = get_tensor_communicator().recv_tensor()
            return hidden_states, residual

        logger.warning(f"=============1 AFDCommunicator prepare_mlp : {get_attention_tp_rank()}, {hidden_states}")
        hidden_states, residual = self.layer_communicator.prepare_mlp(hidden_states, residual, forward_batch)
        logger.warning(f"=============2 AFDCommunicator prepare_mlp : {get_attention_tp_rank()}, {hidden_states}")
        get_tensor_communicator().send_tensor(hidden_states)

        logger.warning(f"=============3 AFDCommunicator prepare_mlp : {get_attention_tp_rank()}, {hidden_states}")
        return hidden_states, residual
...
class AFDProxyMLP(nn.Module):
    def forward(self, hidden_states: torch.Tensor,
        ...
        logger.warning(f"============= AFDProxyMLP : {get_attention_tp_rank()}, {hidden_states}")
        return hidden_states
