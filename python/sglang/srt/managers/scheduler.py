...
class Scheduler(
    ...
    def event_loop_afd(self):
        ...
        logger.info("cll--------- event_loop_afd: role={} m={}".format(get_afd_perspective(), get_afd_mirco_batch()))
        while True:
            # AFD-NOTE: skip warmup batch of ffn, as it will be done by attn warmup
            # AFD TODO: attn forward non-work-request to ffn
            recv_reqs = self.recv_requests()
            self.process_input_requests(recv_reqs)
            if afd_is_ffn() and self.afd_batchsize_attn is None:
                continue
            ...
            if batch:
                ...
                if self.last_batch and afd_is_attn():
                    batch.forward_mode.increase_id(self.last_batch.forward_mode)
                ...
                if afd_is_attn() and self.pp_rank == 0 and self.attn_tp_rank == 0:
                    ...
                    # afd_req.forward_mode.set_id(batch.forward_mode)
                    afd_req.batch_decode_id = batch.forward_mode.batch_decode_id
                    afd_req.batch_prefill_id = batch.forward_mode.batch_prefill_id
                    logger.warning(f"cll ---- {get_afd_perspective()}"
                                   f" send afd req batch:{afd_req.forward_mode.batch_id}"
                                   f" batch_size:{afd_req.batch_size}")
                    self.send_to_ffn.send_pyobj(afd_req)

                prepare_overlap(batch)
                
                if afd_is_ffn():
                    assert self.afd_batchsize_attn == batch.batch_size(), f"batch mismatch {self.afd_batchsize_attn} vs {batch.batch_size()}"
                    
                result = self.run_batch(batch)

                logger.warning(f"cll ---- {get_afd_perspective()} finish current batch:{batch.forward_mode.batch_id}")
                self.process_batch_result(batch, result)
        ...
        if self.pp_rank == 0:
            if self.attn_tp_rank == 0:
                ...
                if afd_is_ffn():
                    while True:
                        try:
                            recv_afd = self.recv_from_attn.recv_pyobj(zmq.NOBLOCK)
                        except zmq.ZMQError:
                            break
    ...
    def  process_input_requests(self, recv_reqs: List):
        ...
        for index, recv_req in enumerate(recv_reqs):
            ...
            logger.warning(f"cll---------- process_input_requests {recv_req.__class__.__name__}")
            ...
            if is_work_request(recv_req):
                ...
                if afd_is_attn() and self.attn_tp_rank == 0:
                    # forward work req to ffn
                    self.send_to_ffn.send_pyobj(recv_req)

            if is_afd_request(recv_req):
                assert afd_is_ffn()
                self.afd_batchsize_attn = recv_req.batch_size
                self.afd_forward_mode = recv_req.forward_mode
                recv_req.forward_mode.batch_prefill_id = recv_req.batch_prefill_id
                recv_req.forward_mode.batch_decode_id = recv_req.batch_decode_id

                logger.warning(f"cll -- {get_afd_perspective()}"
                               f" recv atten req:{recv_req.forward_mode.batch_id}"
                               f" batch_size:{recv_req.batch_size}")


                # ffn recv afd_req, return immediately to run batch
                logger.warning(f"cll------------------------ process_input_requests afd return index:{index} ====== {len(recv_reqs)}") 
                return
    ...
    def get_next_batch_to_run(self) -> Optional[ScheduleBatch]:
        ...
        if new_batch is not None:
            ...
            if not self.running_batch.is_empty():
                if afd_is_ffn():
                    # forcibly finish some reqs to align with attn
                    batch = self.running_batch

                    assert batch.batch_size() >= self.afd_batchsize_attn, "{} < {}".format(batch.batch_size(), self.afd_batchsize_attn)
                    for i in range(batch.batch_size() - self.afd_batchsize_attn):
                        batch.reqs[i].finished_reason = FINISH_LENGTH(0)
    ...
    def run_batch(
        ...
        logger.warning(f"=============== run batch 0 {self.attn_tp_rank}, {batch.batch_size}")
        ...
        logger.warning(f"=============== run batch 1 {self.attn_tp_rank}, {batch.batch_size}")
        # Run forward
        if self.is_generation:
            logger.warning(f"=============== run batch 2 {self.attn_tp_rank}, {batch.batch_size}")
            if self.spec_algorithm.is_none():
            ...
            logger.warning(f"=============== run batch 3 {self.attn_tp_rank}, {batch.batch_size}")
            model_worker_batch = batch.get_model_worker_batch()
